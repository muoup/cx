...
## Standard library documentation generation investigation

Investigation snapshot: 2026-08-25. The Docusaurus site uses the default docs plugin with content under `site/docs`, explicit sidebars in `site/sidebars.ts`, Node 20-compatible tooling, and no existing documentation generator. The production site currently passes local TypeScript checking and `docusaurus build` using the installed site-local binaries.

The CX standard library is a suitable size for a curated structured catalog: `lib/std` contains 13 `.cx` modules and 846 lines. The source has almost no public documentation comments; the comments that exist are implementation TODOs or safety notes. A generator can therefore own presentation and navigation, but the semantic explanations must initially remain manually authored rather than extracted from source comments.

The least invasive design is a semi-generated catalog. Store one machine-readable JSON record per module under `site/stdlib`, validate the records at build time, and generate ordinary Markdown pages under `site/docs/stdlib`. Add the generated directory to the site ignore rules, run the generator from `prestart` and `prebuild`, and expose the generated index through a new standard-library sidebar. JSON is preferable to YAML here because the site has no YAML dependency and Node can parse JSON without changing the lockfile.

Each record should separate the exact display signature from structured semantics: module and source path, public types and variants, qualified function name, signature, template parameters, stage (`runtime` or `comptime`), parameters with names/types/modes/descriptions, return type and explanation, safety/ownership notes, examples, stability, and source location. The signature preserves CX syntax that is difficult to reconstruct safely from generic fields, while the separate fields make the data useful to agents and renderers. The renderer can produce a module page, type sections, function headings, parameter and return blocks, code examples, source links, and a downloadable JSON catalog without a custom Docusaurus plugin.

The generator should check that every declared source path exists, module filenames are unique, qualified names are unique within a module, and source anchors still contain the documented symbol. A later compiler-side extractor can generate the structural signature/visibility/source-location layer from the CX parser and merge it with the manually maintained descriptions; that is the right long-term drift solution, but it is unnecessary for the first documentation sprint. The current library has public/private sections, C helper declarations, generic associated functions, comptime functions, references, and linear cleanup, so a regex-only extractor would produce misleading API pages.

The first implementation slice should cover `io`, `optional`, `string`, `vector`, and `span`, then add `box`, `file`, `cell::rcell`, and networking modules after the schema handles receivers, generic types, enum-union variants, comptime callbacks, and ownership/safety annotations. The generated pages should carry an explicit experimental or stable status because the library has no independent versioning, and CI should run the catalog validator before the existing Docusaurus typecheck/build steps.
## Standard library documentation shell implementation

The initial structured documentation shell is implemented in the working tree. Thirteen module JSON records under `site/stdlib` describe the current public standard-library types, fields, enum-union variants, functions, parameters, return values, and comptime stages with concise placeholder prose. `site/scripts/generate-stdlib-docs.mjs` renders those records as ordinary Markdown pages under the ignored `site/docs/stdlib` directory.

The site now exposes the generated pages through an autogenerated `stdlibSidebar` and a Standard Library navbar item. `prestart`, `prebuild`, and `stdlib:generate` invoke the renderer, so CI and local site builds regenerate pages from the records. The simplified record format uses the scoped module name as its page title, removes parameter mode, safety, and status metadata, and embeds type kinds directly in type names. No standalone validator or source extractor was added; future compiler-driven API extraction can replace or supplement the JSON structural fields without changing the renderer.

The local JSON parse, TypeScript check, direct Docusaurus build, and package-level `npm run build` all pass. The package build generated the Standard Library route and thirteen module routes beneath `build/docs/stdlib`; generated Markdown remains ignored, while the JSON records, generator, navigation, and authoring README are reviewable source changes.
