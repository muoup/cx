...
pub(crate) fn gen_imports(ast: &AST) -> Option<Vec<String>> {
    let mut imports = Vec::new();

    for statement in &ast.statements {
        let GlobalStatement::Import { path } = statement else {
            continue;
        };
...
fn base_type_defs() -> TypeMap {
    ...
    type_defs.insert("signed char".to_owned(), type_defs.get("i8").unwrap().clone());
    type_defs.insert("unsigned char".to_owned(), type_defs.get("u8").unwrap().clone());

    type_defs.insert("void".to_owned(), ValueType::Unit.into());

    let pointer_size = size_of::<usize>() as u8;
    type_defs.insert("usize".to_owned(), ValueType::Integer { bytes: pointer_size, signed: false }.into());
