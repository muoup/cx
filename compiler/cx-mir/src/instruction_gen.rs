use crate::aux_routines::{
    allocate_variable, assign_value, get_cx_struct_field_by_index, try_access_field,
};
...
pub fn generate_instruction(builder: &mut MIRBuilder, expr: &TCExpr) -> Option<MIRValue> {
    match &expr.kind {
        ...
        TCExprKind::Access {
            ...
            bytecode_error_log!(
                builder,
                "Invalid access operation on {} with {field}",
                target._type
            );
        }
        ...
        TCExprKind::FunctionCall {
            ...
            let prototype = match &function._type.kind {
                CXTypeKind::Function { prototype } => *prototype.clone(),
                CXTypeKind::PointerTo {
                    inner_type: inner, ..
                } => {
            ...
            if prototype.return_type.is_structured() {
                ...
                let buffer = builder.add_instruction(
                    ...
                    MIRType::from(MIRTypeKind::Pointer {
                        nullable: false,
                        dereferenceable: buffer_type.fixed_size() as u32,
                    }),
                )?;
        ...
        TCExprKind::ImplicitLoad { operand } => {
            ...
            match &expr._type.kind {
                CXTypeKind::Structured { .. } | CXTypeKind::Union { .. } => Some(inner),
        ...
        TCExprKind::Match {
            ...
            builder.add_instruction(
                VirtualInstruction::JumpTable {
                    value: tag_value,
                    targets: blocks.iter().map(|(tag, block)| (*tag, *block)).collect(),
                    default: default_block.unwrap_or(merge_block),
                },
                MIRType::unit(),
            );
        ...
        TCExprKind::ConstructorMatchIs {
            ...
            let tag_const = builder.match_int_const(
                *variant_tag as i32,
                &MIRType::from(MIRTypeKind::Unsigned { bytes: 4 }),
            );
            ...
            builder.insert_symbol(var_name.as_string(), expr_value);
        ...
        TCExprKind::Break => {
            ...
            builder.add_instruction(VirtualInstruction::Jump { target: merge }, MIRType::unit());
        ...
        TCExprKind::Continue => {
            ...
            builder.add_instruction(VirtualInstruction::Jump { target: cond }, MIRType::unit());

            Some(MIRValue::NULL)
        }

        TCExprKind::SizeOf { _type } => {
            let type_size = builder.convert_cx_type(&expr._type)?.size();

            match type_size {
                MIRTypeSize::Fixed(size) => Some(builder.int_const(size as i32, 8, true)),
                MIRTypeSize::Variable(size_expr) => Some(size_expr),
            }
        }
        ...
        TCExprKind::Taken | TCExprKind::Unit => unreachable!(
            "generate_instruction: Expected expression, found {:?}",
            expr._type.kind
        ),
    }
}
...
pub(crate) fn generate_binop(
    ...
    match op {
        ...
        _ => {
            ...
            generate_algebraic_binop(builder, &lhs._type, left_id, right_id, return_type, op)
        }
    }
}
...
pub(crate) fn generate_algebraic_binop(
    ...
    match &cx_lhs_type.kind {
        ...
        CXTypeKind::Integer { signed: false, .. } => builder.add_instruction(
            VirtualInstruction::IntegerBinOp {
                left: left_id,
                right: right_id,
        ...
        CXTypeKind::Bool => builder.add_instruction(
            VirtualInstruction::IntegerBinOp {
                left: left_id,
                right: right_id,
                op: builder.cx_i_binop(op)?,
        ...
        CXTypeKind::PointerTo { .. } => {
            let CXTypeKind::PointerTo {
                ...
                unreachable!(
                    "generate_binop: Expected pointer type for {left_id}, found {cx_lhs_type}"
                )
            };
        ...
        CXTypeKind::Float { .. } => builder.add_instruction(
        ...
        _type => panic!("Invalid arguments with type for binop not caught by type checker"),
    }
}
