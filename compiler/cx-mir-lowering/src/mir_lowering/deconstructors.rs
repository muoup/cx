...
pub fn generate_deconstructor(builder: &mut LMIRBuilder, mir_type: &MIRType) -> CXResult<()> {
    ...
    let self_value = LMIRValue::ParameterRef(0);
...
pub fn invoke_conditional_deconstruction(
    builder: &mut LMIRBuilder,
    value: &LMIRValue,
    liveness_value: &LMIRValue,
    mir_type: &MIRType,
) -> CXResult<LMIRValue> {
    let liveness = builder.add_new_instruction(
        LMIRInstructionKind::Load {
            memory: liveness_value.clone(),
            _type: LMIRType::from(LMIRTypeKind::Integer(LMIRIntegerType::I1)),
        },
        LMIRType::from(LMIRTypeKind::Integer(LMIRIntegerType::I1)),
        true,
    )?;
...
pub fn allocate_liveness_variable(builder: &mut LMIRBuilder) -> CXResult<LMIRValue> {
...
pub fn needs_deconstruction(builder: &mut LMIRBuilder, mir_type: &MIRType) -> bool {
    if builder.get_deconstructor(mir_type).is_some() || builder.is_deconstructor_pending(mir_type) {
        return true;
    }

    let add_to_list = |builder: &mut LMIRBuilder| {
