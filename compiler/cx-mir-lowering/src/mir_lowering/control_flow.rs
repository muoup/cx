...
use cx_thir::{
    thir::{
        ...
        expression::{THIRExpression, THIRExpressionKind, THIRLocalID, THIRPostcondition},
        pattern::THIRPattern,
    },
...
pub fn lower_if(
    ...
    if !matches!(_result_type.kind, THIRTypeKind::Unit) {
...
pub fn lower_cswitch(
    ...
    for (case_value, _) in cases {
        ...
        if let THIRExpressionKind::IntLiteral(value) = &case_value.kind {
...
pub fn lower_match(
    ...
    for (pattern, _) in arms {
        ...
        let value = match pattern {
            THIRPattern::Integer(value) => *value as u64,
            THIRPattern::TaggedUnionVariant { variant_index, .. } => *variant_index as u64,
            THIRPattern::Float(..) => unreachable!("Float match patterns are not supported"),
        };
...
pub fn lower_return(
    ...
    if has_return_buffer {
        ...
        builder.add_new_instruction(
            LMIRInstructionKind::Return { value: bc_value },
            LMIRType::unit(),
            false,
        )
    }
...
fn is_fall_through(expr: &THIRExpression) -> bool {
    match &expr.kind {
        THIRExpressionKind::Return { .. }
        | THIRExpressionKind::Yield { .. }
        | THIRExpressionKind::Break { .. }
        | THIRExpressionKind::Continue { .. } => false,
        THIRExpressionKind::Unsafe { expression, .. } => is_fall_through(expression),
