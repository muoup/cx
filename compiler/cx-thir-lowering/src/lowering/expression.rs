        ...
        let value = match &expression.kind {
            ...
            THIRExpressionKind::BinaryOperation { lhs, rhs, op } => {
                if matches!(
                    ...
                    super::control_flow::lower_short_circuit(
                        builder,
                        lhs,
                        rhs,
                        op,
                    ...
                    let lowered_op = super::operators::lower_binary_op(builder, op);
            ...
            THIRExpressionKind::UnaryOperation { operand, op } => {
                ...
                builder.emit(MIRInstrKind::UnOp {
                    out,
                    op: super::operators::lower_unary_op(op, &operand._type),
                    operand: lowered,
                });
                MIRValue::Register(out)
            }
            ...
            THIRExpressionKind::BindRegion {
                ...
                let place = if *adopting {
                    if let MIRValue::Place(place) = initial {
                        ...
                        super::memory::assign_operand_to_place(
                            builder,
                            initial,
                            _type,
                            Some(name.clone()),
            ...
            THIRExpressionKind::RegionDuplicate { source } => {
                ...
                if let THIRTypeKind::MemoryReference { inner_type, .. } = &source._type.kind {
                    ...
                    MIRValue::Place(super::memory::assign_operand_to_place(
                        builder,
                        value,
                        &expression._type,
                        None,
            ...
            THIRExpressionKind::MemberAccess {
                ...
                let base = super::memory::ensure_place(builder, base_value, &base._type);
                let type_id = builder.lower_type(&expression._type);
                let out = builder.place(type_id, None, false);
            ...
            THIRExpressionKind::ArrayAccess {
                ...
                let base = super::memory::ensure_place(builder, array_value, &array._type);
            ...
            THIRExpressionKind::PatternIs { lhs, pattern } => {
                super::aggregates::lower_pattern_test(builder, lhs, pattern, &expression._type)?
            }
            ...
            THIRExpressionKind::TaggedUnionGet {
                ...
                let base = super::memory::ensure_place(builder, base_value, &value._type);
                let type_id = builder.lower_type(&expression._type);
                let out = builder.place(type_id, None, false);
            ...
            THIRExpressionKind::TaggedUnionSet {
                ...
                let target_value = lower_expression(builder, target)?;
                let target = super::memory::ensure_place(builder, target_value, &target._type);
            ...
            THIRExpressionKind::Break { cleanups, .. } => {
                super::control_flow::lower_cleanups(builder, cleanups)?;
            ...
            THIRExpressionKind::While {
                ...
                super::control_flow::lower_while(builder, condition, body, *pre_eval)?;
                MIRValue::Constant(MIRConstant::Unit)
            }
            THIRExpressionKind::For {
                ...
                super::control_flow::lower_for(builder, init, condition, increment, body)?;
                MIRValue::Constant(MIRConstant::Unit)
            }
            THIRExpressionKind::CSwitch {
                ...
                super::control_flow::lower_switch(builder, condition, cases, default.as_deref())?;
                MIRValue::Constant(MIRConstant::Unit)
            }
            THIRExpressionKind::Match {
                ...
                default,
                exhaustive,
            } => super::control_flow::lower_match(
                builder,
                condition,
                *subject,
                arms,
            ...
            THIRExpressionKind::Return {
                ...
                if !cleanups.is_empty() {
                    if let (Some(current), Some(ty)) = (value.take(), return_type.as_ref()) {
                        let saved =
                            super::memory::assign_operand_to_place(builder, current, ty, None);
            ...
            THIRExpressionKind::CallFunction {
                ...
                super::calls::lower_call(builder, function, arguments, contract, &expression._type)?
            }
            THIRExpressionKind::TypeConversion {
                ...
                builder.emit(MIRInstrKind::Coerce {
                    out,
                    operand: value,
                    coercion: super::operators::lower_coercion(
                        conversion,
                        &operand._type,
                        &expression._type,
                    ),
