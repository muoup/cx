...
use cx_thir::{
    thir::{
        data::{THIRFunction, THIRFnPrototype, THIRFnSignature, THIRTypeKind},
        expression::{
            THIRExpression, THIRExpressionKind, THIRFnContract, StructInitialization,
            SymbolValueOrigin,
        },
        pattern::THIRPattern,
        r#type::{THIRField, THIRType},
    },
...
fn aggregate_member_layout(
    ...
    match &aggregate_type.kind {
        THIRTypeKind::Union { variants } => {
            ...
            match field {
                THIRField::Standard { .. } => AggregateMemberLayout::Standard { byte_offset: 0 },
                THIRField::Bitfield {
                    integer_type_id,
                    width,
                    ..
        ...
        THIRTypeKind::Structured { fields } => {
            ...
            let mut active_storage: Option<(cx_thir::thir::r#type::THIRTypeID, usize, usize, usize)> =
                None;
...
fn lower_region_duplicate(
    ...
    if let THIRExpressionKind::MemberAccess {
        base,
        member_index,
        aggregate_type,
    } = &source.kind
...
fn lower_bitfield_write(
    ...
    let THIRExpressionKind::MemberAccess {
        base,
        member_index,
        aggregate_type,
    } = &target.kind
...
pub fn lower_expression(builder: &mut LMIRBuilder, expr: &THIRExpression) -> CXResult<LMIRValue> {
    match &expr.kind {
        THIRExpressionKind::BoolLiteral(value) => Ok(LMIRValue::IntImmediate {
            val: if *value { 1 } else { 0 },
            _type: LMIRType::bool(),
        }),

        THIRExpressionKind::IntLiteral(val) => {
        ...
        THIRExpressionKind::FloatLiteral(val) => {
            let bc_type = builder.convert_cx_type(&expr._type);
        ...
        THIRExpressionKind::Unit => Ok(LMIRValue::NULL),
        ...
        THIRExpressionKind::ContractVariable { name, force_param } => {
            if *force_param {
                let idx = builder
                    .current_prototype()
                    .signature()
        ...
        THIRExpressionKind::FunctionReference { name } => Ok(LMIRValue::FunctionRef(name.clone())),
        ...
        THIRExpressionKind::LifetimeEnd {
            variable: _,
            _type: _,
        } => Ok(LMIRValue::NULL),
        THIRExpressionKind::LeakLifetime { expression } => lower_expression(builder, expression),
        THIRExpressionKind::Unsafe { expression } => lower_expression(builder, expression),
        ...
        THIRExpressionKind::PatternIs { lhs, pattern } => {
            ...
            let THIRPattern::TaggedUnionVariant {
                sum_type,
                variant_index,
                inner_local_id,
                ..
...
fn lower_cleanups(builder: &mut LMIRBuilder, cleanups: &[THIRExpression]) -> CXResult<()> {
...
fn lower_call(
    ...
    let signature = match &function._type.kind {
        ...
        THIRTypeKind::PointerTo { inner_type, .. } => {
            ...
            if let THIRTypeKind::Function { signature } = &inner_type.kind {
...
fn lower_call_signature(
    builder: &LMIRBuilder,
    fn_val: &LMIRValue,
    semantic_signature: &THIRFnSignature,
...
fn lower_call_argument(
    ...
    match abi_arg {
        ...
        LMIRParameterABI::Indirect { alignment } => {
            if let THIRExpressionKind::RegionDuplicate { source } = &arg.kind {
...
fn call_argument_region_source(arg: &THIRExpression) -> &THIRExpression {
    match &arg.kind {
        THIRExpressionKind::RegionDuplicate { source }
        | THIRExpressionKind::RegionMove { source } => source,
        _ => arg,
    }
}
...
pub(crate) fn lower_contract_assertion(
    ...
    message: &str,
    assertion_prototype: &THIRFnPrototype,
) -> CXResult<()> {
...
fn lower_array_initializer(
    builder: &mut LMIRBuilder,
    elements: &[THIRExpression],
    element_type: &cx_thir::thir::data::THIRType,
) -> CXResult<LMIRValue> {
    let bc_element_type = builder.convert_cx_type(element_type);
...
fn lower_struct_initializer(
    ...
    struct_type: &cx_thir::thir::data::THIRType,
) -> CXResult<LMIRValue> {
...
pub fn lower_function(builder: &mut LMIRBuilder, mir_fn: &THIRFunction) -> CXResult<()> {
