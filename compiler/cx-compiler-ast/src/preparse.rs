use cx_data_ast::{parse::parser::VisibilityMode, preparse::naive_types::ModuleResource, try_next, PreparseContents};
use cx_data_lexer::{keyword, operator, punctuator, specifier, TokenIter};
use cx_util::CXResult;

use crate::{declarations::{decl_parsing::{parse_import, parse_plain_typedef, parse_typedef, try_function_parse}, type_parsing::parse_initializer}, parsing_tools::goto_statement_end};
...
pub(crate) fn preparse_global_expr(data: &mut PreparseData) -> CXResult<()> {
    ...
    let Some(method) = try_function_parse(&mut data.tokens, return_type, name.clone()).flatten() else {
