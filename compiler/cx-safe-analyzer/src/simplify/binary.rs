...
pub fn eval_int_binop(op: &FMIRIntrinsicIBinOp, left: i64, right: i64) -> Option<ConstValue> {
    ...
    match op {
        ...
        Op::LT | Op::ILT => Some(ConstValue::Bool(left < right)),
        Op::LE | Op::ILE => Some(ConstValue::Bool(left <= right)),
        Op::GT | Op::IGT => Some(ConstValue::Bool(left > right)),
        Op::GE | Op::IGE => Some(ConstValue::Bool(left >= right)),
