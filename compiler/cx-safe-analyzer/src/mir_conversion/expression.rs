...
fn dummy_counter() -> u64 {
    static ATOMIC_COUNTER: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
    ATOMIC_COUNTER.fetch_add(1, std::sync::atomic::Ordering::SeqCst)
}

pub fn convert_expression(env: &mut FMIREnvironment, mir_expr: &MIRExpression) -> FMIRNode {
    match &mir_expr.kind {
        ...
        MIRExpressionKind::Block { statements } => {
            ...
            statements
                .iter()
                .map(|stmt| convert_expression(env, stmt))
                .rfold(None, |acc, stmt| match acc {
                    None => Some(stmt),
                    Some(prev) => {
        ...
        MIRExpressionKind::If {
            ...
            let then_node = convert_expression(env, then_branch);
            ...
            FMIRNode {
                ...
                body: FMIRNodeBody::If {
                    condition: FRc::new(convert_expression(env, condition)),
        ...
        MIRExpressionKind::While {
            ...
            let condition_node = FRc::new(convert_expression(env, condition));
            let body_node = FRc::new(convert_expression(env, body));
        ...
        MIRExpressionKind::For {
            ...
            let loop_effect = init_node
                ._type
                .union(&condition_node._type)
                .union(&increment_node._type)
                .union(&body_node._type);

            let effect_monad = loop_effect.apply(FMIRType::pure(MIRType::unit()));

            FMIRNode {
                _type: effect_monad.clone(),
                body: FMIRNodeBody::CLoop {
        ...
        MIRExpressionKind::Variable(name) => FMIRNode {
        ...
        MIRExpressionKind::MemoryRead { source } => {
            ...
            let capture = CXIdent::from(format!("__{}", dummy_counter()));
                    ...
                    function: FRc::new(FMIRNode {
                        ...
                        body: FMIRNodeBody::Load {
                            pointer: FRc::new(FMIRNode {
                                body: FMIRNodeBody::VariableAlias {
                                    name: capture.as_string(),
                                },
                                _type: FMIRType::pure(source._type.clone()),
                            }),
                        },
                    }),
                },
                    ...
                    function: FRc::new(FMIRNode {
                        ...
                        body: FMIRNodeBody::Store {
                            pointer: FRc::new(FMIRNode {
                                ...
                                _type: FMIRType::pure(target._type.clone()),
                            }),
