...
use cx_mir::{
    MIRAggregateOp, MIRBlockTarget, MIRComptimeInstrKind, MIRComptimeOp, MIRConstant,
    MIRFunctionID, MIRFunctionMode, MIRInstrKind, MIRIntType, MIRTarget, MIRUnaryOp, MIRValue,
};
...
fn push_frame<'ctx>(
    ...
    let mut frame = Frame::new(code, engine.next_frame);
    engine.next_frame += 1;
    for declaration in code.places() {
        frame.cells.insert(
            MIRTarget::Place(declaration.id),
            MIRComptimeValue::Constant(MIRConstant::Undefined),
        );
    }
    for (place, value) in code.parameters().iter().zip(args) {
        frame.cells.insert(MIRTarget::Place(*place), value.clone());
    }
                ...
                None => {
                    return comptime_error(
                        ...
                        (
                            ...
                            ("terminating instruction".into(), "comptime block".into()),
                        ),
                    );
                }
            }
            ...
            MIRComptimeInstrKind::Standard(kind) => match kind {
                ...
                MIRInstrKind::Copy { out, source, ty } => {
                    let value = if engine
                        ...
                        .unwrap_or(false)
                        && memory::reference_parameter(engine, engine.frames.len() - 1, source)
                    {
                        memory::read_value(engine, &MIRValue::Reference(source))?
                    } else {
                ...
                MIRInstrKind::Intrinsic(_) => {
                    return comptime_error(
                        range,
                        (
                            &catalogue::COMPTIME_INVALID_OPERATION,
                            "variadic operation".into(),
                        ),
                    );
                }
                ...
                MIRInstrKind::UnOp { out, op, operand } => {
                    if let MIRUnaryOp::Increment { amount, post } = op {
                        let old = match operand {
                            MIRValue::Reference(target) => {
                                match memory::read_target(engine, target)? {
                                    MIRComptimeValue::Constant(value) => value,
                                    _ => {
                                        return comptime_error(
                                            range,
                                            (
                                                &catalogue::COMPTIME_INVALID_OPERATION,
                                                "increment a nonconstant value".into(),
                                            ),
                                        );
                                    }
                                }
                            }
                            _ => memory::read_constant(engine, &operand, &range)?,
                        };
                    ...
                    match taken {
                        ...
                        None => {
                            return comptime_error(
                                ...
                                (
                                    &catalogue::MISSING_ENTITY,
                                    ("matching case".into(), "comptime integer switch".into()),
                                ),
                            );
                        }
                    }
                    ...
                    match taken {
                        ...
                        None => {
                            return comptime_error(
                                ...
                                (
                                    &catalogue::MISSING_ENTITY,
                                    ("matching case".into(), "comptime variant switch".into()),
                                ),
                            );
                        }
                    }
            ...
            MIRComptimeInstrKind::Comptime(MIRComptimeOp::MakeStaged {
                ...
                for capture in captures {
                    let value = match capture {
                        MIRValue::Reference(target) => memory::read_target(engine, target)?,
                        _ => memory::read_value(engine, &capture)?,
                    };
                    bindings.push(MIRStagedBinding::Comptime(value));
                }
                ...
                let MIRComptimeValue::Staged(staged) = memory::read_value(engine, &staged)? else {
                    return comptime_error(
                        ...
                        (
                            ...
                            (
                                "staged application".into(),
                                "a staged value".into(),
                                Some("non-staged value".into()),
                            ),
                        ),
                    );
                };
...
fn execute_aggregate_op(
    ...
    match op {
        MIRAggregateOp::Target { out, op } => {
            use cx_mir::MIRTargetAggregateOp as Op;
        ...
        MIRAggregateOp::Value { out, op } => {
            ...
            let constant = match op {
                Op::Discriminant { value, .. } => {
                    let value = memory::read_aggregate(engine, &value)?;
