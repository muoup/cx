...
mod tests {
    use super::*;
    use cx_tokens::token::{KeywordType, OperatorType, SpecifierType};

    #[test]
    fn c_mode_keeps_cx_words_as_identifiers() {
        let tokens = tokenize_text(
            "where move match safe public private",
            Path::new("keywords.c"),
            LanguageMode::C,
        )
        .unwrap_or_else(|_| panic!("failed to tokenize C-mode identifiers"));
        let names = tokens
            .into_iter()
            .map(|token| match token.kind {
                TokenKind::Identifier(name) => name,
                other => panic!("expected C identifier, found {other:?}"),
            })
            .collect::<Vec<_>>();
    ...
    #[test]
    fn cx_mode_preserves_cx_tokens() {
        let tokens = tokenize_text(
            "where move match safe public",
            Path::new("keywords.cx"),
            LanguageMode::Cx,
        )
        .unwrap_or_else(|_| panic!("failed to tokenize CX-mode keywords"));
        let kinds = tokens
            .into_iter()
        ...
        assert!(matches!(
            kinds.as_slice(),
            [
                TokenKind::Keyword(KeywordType::Where),
                TokenKind::Operator(OperatorType::Move),
                TokenKind::Keyword(KeywordType::Match),
                TokenKind::Keyword(KeywordType::Safe),
