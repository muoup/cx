use cx_log::CXResult;
use cx_log::catalogue::parse::INVALID_LITERAL;
use cx_tokens::token::{
...
fn hexadecimal_float_literal(iter: &mut LexCursor<'_>, start_index: usize) -> CXResult<TokenKind> {
    ...
    let value = match parse_hexadecimal_float(number) {
        Ok(value) => value,
        Err(_) => return iter.log_error(start_index, &INVALID_LITERAL, ("numeric".into(), Some(number.to_string()))),
    };
...
fn parse_float_literal(iter: &mut LexCursor<'_>, start_index: usize) -> CXResult<TokenKind> {
    ...
    match value {
        ...
        Err(_) => iter.log_error(start_index, &INVALID_LITERAL, ("numeric".into(), Some(num.to_string()))),
    }
}
...
fn parse_integer_literal(
    ...
    match u64::from_str_radix(digits, radix) {
        ...
        Err(_) => iter.log_error(
            digits_start,
            &INVALID_LITERAL,
            ("numeric".into(), Some(iter.source()[digits_start..iter.cursor()].to_string())),
        ),
    }
}
...
fn invalid_numeric_literal(iter: &LexCursor<'_>, start_index: usize) -> CXResult<TokenKind> {
    iter.log_error(
        start_index,
        &INVALID_LITERAL,
        ("numeric".into(), Some(iter.source()[start_index..iter.cursor()].to_string())),
    )
}
