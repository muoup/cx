...
pub fn compatible_types(
    ...
    match (&type1.kind, &type2.kind) {
        (MIRTypeKind::PointerTo { inner_type: inner1, .. }, MIRTypeKind::PointerTo { inner_type: inner2, .. }) => {
            let inner1 = env.type_context.get(*inner1).cloned().unwrap();
            let inner2 = env.type_context.get(*inner2).cloned().unwrap();
            
            compatible_types(env, &inner1, &inner2)
        },
        
        (MIRTypeKind::Array { inner_type: inner1, length: len1 }, MIRTypeKind::Array { inner_type: inner2, length: len2 }) => {
            if len1 != len2 {
                return Ok(false);
            } 
       ...
       (MIRTypeKind::Structured { fields: fields1, .. }, MIRTypeKind::Structured { fields: fields2, .. }) |
       (MIRTypeKind::TaggedUnion { variants: fields1, .. }, MIRTypeKind::TaggedUnion { variants: fields2, .. }) |
       (MIRTypeKind::Union { variants: fields1, .. }, MIRTypeKind::Union { variants: fields2, .. }) => {
            ...
            return Ok(
                fields1.iter().zip(fields2.iter())
                    .all(|((_, field_type1), (_, field_type2))| {
                        let field_type1 = env.type_context.get(*field_type1).cloned().unwrap();
                        let field_type2 = env.type_context.get(*field_type2).cloned().unwrap();
