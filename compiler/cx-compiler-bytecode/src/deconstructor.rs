...
pub fn deconstruct_variable(builder: &mut BytecodeBuilder, var: ValueID, _type: &CXType) -> Option<()> {
    match &_type.kind {
        CXTypeKind::Structured { .. } => {
            if let Some(deconstructor) = get_deconstructor(builder, _type) {
                builder.call(&deconstructor, vec![var])?;
            }
        },
        ...
        CXTypeKind::StrongPointer { inner_type, is_array } => {
            ...
            let inner_val = builder.add_instruction(
                VirtualInstruction::Load { value: var },
            ...
            match (deconstructor, is_array) {
                (Some(prototype), true) => {
                    ...
                    let deconstructor = builder.fn_ref(&prototype)?.unwrap();
